...
public class ItemsEmitter {
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < items.length; i++) {
            if (items[i].isActive()) {
                batch.draw(regions[items[i].getItemType().index], items[i].getPosition().x - 16, items[i].getPosition().y - 16);
            }
        }
    }
    ...
    public void generateRandomItem(float x, float y, int count, float probability) {
        for (int q = 0; q < count; q++) {
            ...
            if (n <= probability) {
                int type = MathUtils.random(0, ItemType.values().length - 1);
                for (int i = 0; i < items.length; i++) {
                    if (!items[i].isActive()) {
                        items[i].setup(x, y, ItemType.values()[type]);
                        break;
                    }
                }
            }
